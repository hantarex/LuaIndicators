...
function OnInit()
    config = jsonStorage.loadTable(getScriptPath().."/config/config.cfg")
...
--    iup.MainLoop()
    ...
    myTrade = TradeCondition(config.fees, config.needProfit, config.stopOrder, config.speedTrade, config.isTraiding, config.needBestProfit)
    myTrade:setDSInfo({
